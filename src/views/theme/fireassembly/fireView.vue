...
<template>
 <div class="fireView">
    <div @click="$router.push('/home')" class="topclass topbar"></div>
    <div class="main">
      <gisMain></gisMain>
    </div>
    <div class="main_right">
      <div
        class="main_right_top"
        :class="{ lang: rightTopIndex > 0 }"
        v-show="rightTopIndex > 0"
       >
          ...
          <DisStat :chainData="data.chain" @setPanelView="setPanelView" v-if="rightTopIndex === 3"></DisStat>
        </div>
    </div>
  </div>
</template>
...
import DisStat from "@/views/theme/fireassembly/DisasterStatistics.vue"; //echart图展示
import extract from "@/views/theme/fireassembly/extract.vue"; // 应急语义提取
import chain from "@/views/theme/fireassembly/chain.vue"; // 逻辑链
@Component({
  name: "fireView",
  components: {
    gisMain,
...
.fireView {
  ...
  .topbar {
    ...
    position: absolute;
    top: 0;
    background: url(~"@{img}/model_topTitle_bg.png") 50% center / 100% 100%
      no-repeat;
    line-height: 110px;
    font-size: 36px;
  }
...
.fire_top {
  height: 50px;
  //background: url(~"@{img}/disaster_model_serve.png") no-repeat left;
}
.fire_title {
  height: 60px;
  line-height: 60px;
